...
func(s *Server)list(w http.ResponseWriter,r *http.Request){
    ...
    if q!=""||len(filters)>0{wj(w,200,map[string]any{"services":oe(s.db.Search(q,filters))});return}
    wj(w,200,map[string]any{"services":oe(s.db.List())})
}
...
func(s *Server)update(w http.ResponseWriter,r *http.Request){
    ...
    var patch store.Service;json.NewDecoder(r.Body).Decode(&patch);patch.ID=existing.ID;patch.CreatedAt=existing.CreatedAt
...
func(s *Server)health(w http.ResponseWriter,r *http.Request){wj(w,200,map[string]any{"status":"ok","service":"hub","services":s.db.Count()})}
